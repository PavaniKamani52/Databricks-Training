...
###Q46. find employee(s) with highest salary
select *
from Employee
...
###Q49. find department with most employees
select department_id
from Employee
...
###Q53. find departments where average salary > 55000
select d.name
from Department d
join Employee e
on d.department_id = e.department_id
...
###Q58. departments with more than 1 employee and avg salary > 55000
select department_id
from Employee
group by department_id
...
###Q60. total employees and avg salary for departments with more than 2 employees
select department_id, count(*) as total_employees, avg(salary) as avg_salary
from Employee
group by department_id
having count(*) > 2;

###Q61. names and salary of employees earning above their department average
select name, salary
from Employee e
where salary > (
    select avg(salary)
...
###Q62. employees hired on same date as the oldest employee (earliest hire_date)
select name
from Employee
where hire_date = (
...
###Q63. department names with total number of projects, ordered by project count
...
###Q64. employee name with highest salary in each department
...
###Q65. names and salaries of employees older than department average age
select name, salary
from Employee e
where age > (
    select avg(age)
